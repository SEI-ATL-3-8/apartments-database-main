...
-- SELECT MAX(age) AS Oldest FROM owners;
-- SELECT age from owners ORDER BY age desc LIMIT 1;

-- 10. Show all the properties that are owned by the owner with an id of 2.
--  SELECT * FROM properties WHERE ownerid = 2;

-- 11. Show all the properties that are owned by Yuki.
-- SELECT properties.name FROM owners JOIN properties ON owners.id = properties.ownersId WHERE owners.name ='Yuki';
...
-- SELECT COUNT (name) FROM properties WHERE ownersId = 6;

-- 13. Show the name of Willowspring's owner.
-- SELECT owners.name FROM owners JOIN properties on owners.id = properties.ownerid WHERE properties.name = 'Willowspring';

-- 14. Show the names of the people who own properties that have names of 10 characters or longer
--  SELECT owners.name FROM owners JOIN properties on owners.id = properties.ownerid WHERE LENGTH (properties.name) > 10;
